...
def find_indices_within_bounds(
    X: np.array, min_vector: np.array, max_vector: np.array
) -> List[int]:
    """Return indices for rows of X that are between the min and max vector values

    Args:
        X (np.array): n x d array of points
        min_vector (np.array): n points describing minimum allowed value per column d
        max_vector (np.array): n points describing maximum allowed value per column d

    Returns:
        List[int]: list of row indices respecting min and max vectors
    """
    idx_within_bounds = np.logical_and(
        (X >= min_vector).all(axis=1), (X <= max_vector).all(axis=1)
    )
    return idx_within_bounds
...
def discrete_LHS(
    ...
    """Uniformly sample n points from discrete set X.

    Args:
        X (np.array): set of points to sample from. Each row is a point, columns are features
        n (int): number of points to sample
        allowed_indices (Optional[List[int]]): row indices of points in X that can be sampled. If None, assume we can sample from all points
        return_indices (bool): if True, return the indices of the sampled points. if False, return the sampled points themselves.

    Returns:
        np.array: points to test. Each row is a point, columns are features. Points are taken from X
        or
        np.array: array of booleans corresponding to a rows in X to test
    """
    if allowed_indices is None:
        allowed_indices = list(range(X.shape[0]))
    else:
        if min(allowed_indices) < 0 or max(allowed_indices) >= X.shape[0]:
            raise ValueError("Allowed indices must be between 0 and X.shape[0]!")

    num_possible = len(allowed_indices)
    if n > num_possible:
        raise ValueError(
            f"Cannot sample more points ({n}) than there are in the set ({num_possible})!"
        )
    scaler = MinMaxScaler()
    X_scaled = scaler.fit_transform(X)
    boxes = diversipy.psa_partition(X_scaled, n, allowed_indices)
    indices = np.array([box.obtain_representative_index() for box in boxes])
    if return_indices:
        return indices
    else:
        return X[indices]
...
def progress_bar(n, T, inc, ensemble, text=""):
    ...
    if p >= inc:
        sys.stdout.write(
            f"\r{'Ensemble '+str(ensemble + 1)+', '+text+' : ['+block*step + space*(total-step)+'] '}{n + 1} / {T} experiments completed . . .{' '*step}"
        )
        sys.stdout.flush()
        inc += 0.1
        ...
        step = int(total)
        sys.stdout.write(
            f"\r{'Ensemble '+str(ensemble + 1)+', '+text+' : ['+block*step + space*(total-step)+']'} {T} / {T} Complete!{' '*step}"
        )
        sys.stdout.flush()
        inc += 0.1
    return inc
...
def plot(fX_min, compute, nregular, bound_l, bound_u, dim, ensemble, activations):
    ...
    ax1.fill_between(
        xs[nregular:], fX_min10[nregular:], fX_min90[nregular:], color="b", alpha=0.2
    )
